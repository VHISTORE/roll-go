...
const timeTypeRadios = document.querySelectorAll('input[name="delivery-time-type"]');
...
// Генерация слотов каждые 30 минут с 15:30 до 22:00
function generateTimeSlots() {
    ...
    while (startHour < endHour || (startHour === endHour && startMin === 0)) {
        let h = startHour.toString().padStart(2, '0');
        let m = startMin.toString().padStart(2, '0');
        slots.push(`${h}:${m}`);
    ...
    preorderTimeSelect.innerHTML = slots.map(time => `<option value="${time}">${time}</option>`).join('');
}

// Переключение видимости выбора времени
timeTypeRadios.forEach(radio => {
    radio.addEventListener('change', (e) => {
        if (e.target.value === 'scheduled') {
            scheduledTimeWrapper.style.display = 'block';
            generateTimeSlots();
        } else {
            scheduledTimeWrapper.style.display = 'none';
        }
    });
});

// --- Функции корзины и заказа ---
...
async function sendOrderToTelegram(paymentMethod, status = "NEW ORDER 🍣") {
    ...
    const timeType = document.querySelector('input[name="delivery-time-type"]:checked').value;
    const deliveryTime = timeType === 'asap' ? 'ASAP ⚡' : `Scheduled: ${preorderTimeSelect.value} ⏰`;
...
function resetFullState() {
    ...
    scheduledTimeWrapper.style.display = 'none';
    document.querySelector('input[value="asap"]').checked = true;
